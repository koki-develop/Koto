...
**Startup does the expensive work.** `KotoInputController.preload()` builds the converter *and* the candidate window at launch, when nothing is waiting on us. Both are shared and built lazily, so whatever `preload()` leaves out gets built on first touch instead — and first touch is mid focus-transition either way: `activateServer` for the candidate window (`NSPanel` plus Auto Layout), `deactivateServer` or the first keystroke for the converter (the dictionary load).
